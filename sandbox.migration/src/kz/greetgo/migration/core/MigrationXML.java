...
public class MigrationXML extends Migration {
    ...
    private void addClientToBatch(PreparedStatement ps,ClientRecord clientRecord) throws SQLException{
        ps.setLong(1, clientRecord.number);
        ps.setString(2, clientRecord.id);
        ps.setString(3, clientRecord.surname);
        ps.setString(4, clientRecord.name);
        ps.setString(5, clientRecord.patronymic);
        ps.setDate(6, clientRecord.birthDate);
        ps.setLong(7, clientRecord.charm.id);
        ps.setString(8, clientRecord.gender.toString());
        ps.addBatch();
    }
    private void addAddressToBatch(PreparedStatement ps,AddressRecord addressRecord) throws Exception{
        ps.setLong(1, addressRecord.client_number);
        ps.setString(2, addressRecord.client_id);
        ps.setString(3, addressRecord.type.name());
        ps.setString(4, addressRecord.street);
        ps.setString(5, addressRecord.house);
        ps.setString(6, addressRecord.flat);
        ps.addBatch();
    }
    private void addPhoneToBatch(PreparedStatement ps,Phone phoneRecord) throws Exception{
        ps.setLong(1, phoneRecord.client_number);
        ps.setString(2, phoneRecord.client_id);
        ps.setString(3, phoneRecord.type.name());
        ps.setString(4, phoneRecord.number);
        ps.addBatch();
    }
    ...
    private int download() throws SQLException, IOException, SAXException {
        ...
        try (PreparedStatement ciaPS = ciaConnection.prepareStatement(
            ...
            PreparedStatement operClPS = null;
            PreparedStatement operClAdPs = null;
            PreparedStatement operClPhPs = null;


            int batchSize = 0, recordsCount = 0;

            try (ResultSet ciaRS = ciaPS.executeQuery()) {
                operClPS = getOperConPrStmnt(r(insertClient.getSQL()));
                operClAdPs = getOperConPrStmnt(r(insertAddress.getSQL()));
                operClPhPs = getOperConPrStmnt(r(insertPhone.getSQL()));
                ...
                while (ciaRS.next()) {
                    ...
                    client.number = ciaRS.getLong("number");
                    client.parseRecordData(ciaRS.getString("record_data"));
                    client.charm.id = charmDictionary.get(client.charm.name) == null ? -1L : charmDictionary.get(client.charm.name);

                    //Записываем адреса во временную таблицу
                    if (client.addressRecordList != null && !client.addressRecordList.isEmpty()) {
                        for (AddressRecord addressRecord : client.addressRecordList) {
                            addAddressToBatch(operClAdPs, addressRecord);
                        }
                    }

                    if (client.phoneList != null && !client.phoneList.isEmpty()) {
                        for (Phone phone : client.phoneList) {
                            addPhoneToBatch(operClPhPs, phone);
                        }
                    }
                    addClientToBatch(operClPS, client);
                    batchSize++;
                    recordsCount++;
