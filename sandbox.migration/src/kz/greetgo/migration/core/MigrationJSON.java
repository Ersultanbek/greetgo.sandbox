...
public class MigrationJSON extends Migration {
    ...
    private void addAccountToBatch(PreparedStatement ps, Account account) throws SQLException {
        ps.setString(1, account.client_id);
        ps.setString(2, account.account_number);
        ps.setDate(3, account.registered_at);
        ps.setLong(4, account.number);
        ps.addBatch();
    }

    private void addTransactionToBatch(PreparedStatement ps, Transaction transaction) throws SQLException {
        ps.setString(1, transaction.account_number);
        ps.setString(2, transaction.transaction_type);
        ps.setDate(3, transaction.finished_at);
        ps.setObject(4, transaction.money);
        ps.setLong(5, transaction.number);
        ps.addBatch();
    }
    ...
    public int download() throws SQLException, IOException, SAXException {
        ...
        try (PreparedStatement ciaAcTrPS = ciaConnection.prepareStatement(
            ...
            PreparedStatement operAcPS = null;
            PreparedStatement operTrPS = null;
            ...
            try (ResultSet ciaRS = ciaAcTrPS.executeQuery()) {
                ...
                long startedAt = System.nanoTime();
                operAcPS = getOperConPrStmnt(r(insertAccount.getSQL()));
                operTrPS = getOperConPrStmnt(r(insertTransaction.getSQL()));
                ...
                while (ciaRS.next()) {
                    ...
                    if (TYPE_ACCOUNT.equals(type)) {
                        ...
                        addAccountToBatch(operAcPS, account);
                    }
                    if (TYPE_TRANSACTION.equals(type)) {
                        ...
                        addTransactionToBatch(operTrPS, transaction);
                    }
                }
